...
const waifuCommand: I_Command = {
	...
	async execute(interaction) {
		try {
			...
			const compositeImageBuffer = await createFakeTinderCardHorizontal(images[currentIndex], waifu, currentIndex, images.length);
			...
			collector.on("collect", async (i) => {
				...
				const newCompositeImageBuffer = await createFakeTinderCardHorizontal(images[currentIndex], waifu, currentIndex, images.length);
			...
			// When our time to collect is up, we delete the message.
